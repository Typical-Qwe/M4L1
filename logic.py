...
class DatabaseManager:
    ...
    def get_random_prize(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM prizes WHERE used = 0 ORDER BY RANDOM()"
            )
    ...
    def get_winners_img(self, user_id):
        ...
        with conn:
            ...
            cur.execute(''' 
                ...
                INNER JOIN prizes 
                ON winners.prize_id = prizes.prize_id
                WHERE user_id = ?
            ''', (user_id,))
            return cur.fetchall()
...
def hide_img(img_name):
    ...
    pixel = cv2.resize(small, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_NEAREST)
